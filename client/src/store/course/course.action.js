...
import { addCourse, deleteCourse, getCourse, updateCourse } from '../api';

const getAll = (courseArray) => createAction(COURSE_ACTION_TYPES.GET_ALL_COURSE, courseArray);
...
const updateOne = () => createAction(COURSE_ACTION_TYPES.UPDATE_COURSE);
...
export const updateOneCourse = (id, courseToUpdate) => {
	return (dispatch) => {
		const resp = updateCourse(id, courseToUpdate);
		resp
			.then(function(response) {
				// console.log(response);
				dispatch(updateOne());
